...
export default function AuthorityDashboard() {
  ...
  const fetchDashboardStats = async () => {
    try {
      ...
      setError(err.message || 'Unable to load authority dashboard data.');
    }
  };
  ...
  const fetchComplaints = async () => {
    ...
    try {
      ...
      console.error('[AUTHORITY DASHBOARD] Failed to load complaints:', err);
    } finally {
  ...
  const handleCardClick = (filterKey) => {
    if (activeStatusFilter === filterKey && filterKey !== 'Total Complaints') {
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: '16px', marginBottom: '32px' }}>
        <div>
          <div style={{ display: 'inline-flex', alignItems: 'center', gap: '6px', fontSize: '0.8125rem', color: 'var(--color-primary)', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.04em', marginBottom: '6px' }}>
            <Building2 size={14} />
            <span>Municipal Grievance Administration Console</span>
          </div>
          <h1 className="page-title">Authority Dashboard</h1>
          <p className="page-subtitle">
            Welcome, <strong>{user?.name}</strong> ({user?.designation || user?.department || 'Municipal Authority'})
            {user?.role === 'authority_category' && user?.assignedCategory && (
              <span style={{ display: 'inline-block', marginLeft: '8px', padding: '2px 8px', borderRadius: '4px', backgroundColor: 'var(--color-primary-light)', color: 'var(--color-primary)', fontWeight: 600, fontSize: '0.75rem' }}>
                Scoped: {user.assignedCategory}
              </span>
            )}
          </p>
        </div>
      ...
      <div style={{ marginBottom: '8px' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '10px' }}>
          <span style={{ fontSize: '0.8125rem', color: 'var(--text-secondary)', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.04em' }}>
            Workflow Status KPI (Click card to filter complaints list)
          </span>
      ...
      <div style={{ backgroundColor: 'var(--bg-surface)', border: '1px solid var(--border-subtle)', borderRadius: '8px', padding: '24px' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px', flexWrap: 'wrap', gap: '14px' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
            ...
            <label htmlFor="authority-sort-order" style={{ fontSize: '0.8125rem', color: 'var(--text-secondary)', fontWeight: 600 }}>
              Sort:
            </label>
            <select
              id="authority-sort-order"
              value={sortBy}
                ...
                complaintsList.map((c) => {
                  ...
                  return (
                    <tr key={c._id}>
                      ...
                      <td>
                        <Link to={`/authority/complaints/${c._id}`} className="btn btn-secondary btn-sm">
                          Manage
                        </Link>
                      </td>
                    </tr>
