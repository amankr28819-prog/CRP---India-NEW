  ...
  if (loading && !data) {
    return (
      <div className="container" style={{ padding: '64px 20px', textAlign: 'center', color: 'var(--text-secondary)' }}>
        <p>Loading Municipal Authority metrics...</p>
      </div>
    );
  }
    ...
    <div className="container" style={{ padding: '36px 20px 80px 20px' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: '16px', marginBottom: '32px' }}>
        <div>
          ...
          <h1 className="page-title">Executive Grievance Dashboard</h1>
          <p className="page-subtitle">
            Welcome, <strong>{user?.name || 'Officer'}</strong> ({user?.designation || 'Zonal Engineer'}) — {user?.department || 'Civic Infrastructure Division'}
          </p>
        </div>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px', flexWrap: 'wrap', gap: '10px' }}>
          <div>
            ...
            <p style={{ fontSize: '0.8125rem', color: 'var(--text-secondary)' }}>
              Latest civic grievances requiring review, field dispatch, or status updates
            </p>
          </div>

          <Link to="/authority/complaints" style={{ fontSize: '0.875rem', color: 'var(--color-primary)', fontWeight: 500, display: 'inline-flex', alignItems: 'center', gap: '4px' }}>
